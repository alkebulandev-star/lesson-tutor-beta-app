...
// Admin email whitelist — these emails get admin role even if the DB
// hasn't been updated yet. Belt-and-braces fail-safe so the app builder
// is never locked out of any section.
var ADMIN_EMAILS = [
  'alkebulandev@gmail.com'
];

function isAdminByWhitelist(profile){
...
function patchRoleGating(){
  ...
  window.goTo = function(id){
    ...
    // Promote whitelisted emails to admin even if the DB hasn't been updated
    if (profile && isAdminByWhitelist(profile)) {
      role = 'admin';
      profile.role = 'admin';
    }
    ...
    // 3. Signed in but no profile yet (cloud still hydrating) → wait.
    if (!profile) {
      ...
      // Safety: if hydrated event already fired, retry after a short timeout
      // so the user isn't stuck on landing forever.
      setTimeout(function(){
        window.removeEventListener('lt-cloud-hydrated', hydrateOnce);
        if (window._LT_LAST_PROFILE) {
          window.goTo(pending);
        } else {
          // Profile still missing — just show the page they requested.
          origGoTo.call(null, pending);
        }
      }, 2000);
      return;
    }

    // ★ ADMIN BYPASS — admins can access every page with no restrictions.
    if (role === 'admin') {
      return origGoTo.apply(this, arguments);
    }
...
function showSignInPrompt(targetPage){
  ...
  if (window.LTAuthUI && typeof window.LTAuthUI.open === 'function') {
    try {
      ...
      setTimeout(function(){
        window.LTAuthUI.open({
          mode: 'signin',
          onAuth: function(){
            // After successful auth, navigate to the page the user wanted.
            if (targetPage && typeof window.goTo === 'function'){
              // Small delay so cloudHydrate finishes and _LT_LAST_PROFILE is set.
              setTimeout(function(){ window.goTo(targetPage); }, 600);
            }
          }
        });
      }, 50);
