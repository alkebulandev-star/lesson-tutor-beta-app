...
(function(){
  ...
  window.toggleSymbolPicker = function(){
    ...
    if (!p) return;
    var existingBackdrop = document.getElementById('symbolPickerBackdrop');
    if (p.style.display === 'none' || !p.style.display){
      p.style.display = 'block';
      renderSymbolGrid(_currentTab);
      // Add a backdrop so users can tap outside to close
      if (!existingBackdrop) {
        var backdrop = document.createElement('div');
        backdrop.id = 'symbolPickerBackdrop';
        backdrop.className = 'symbol-picker-backdrop';
        backdrop.onclick = function(){ window.toggleSymbolPicker(); };
  ...
  // ═══════════════════════════════════════════════════════════════
  // DIAGRAM PLACEHOLDER + MODAL EDITOR
  // ───────────────────────────────────────────────────────────────
  // New flow: the inline block shows only a thumbnail / "Tap to draw"
  // placeholder. Tapping it opens a full-screen drawing modal with the
  // same drawing tools. A "Place on Sheet" button closes the modal and
  // updates the thumbnail. Tapping the thumbnail again re-opens it for
  // editing.
  // ═══════════════════════════════════════════════════════════════
  function renderDiagramToCanvasOffscreen(history, width, height){
    ...
    (history||[]).forEach(function(s){
      x.save();
      x.strokeStyle = s.color || '#1e3a8a';
      x.fillStyle = s.color || '#1e3a8a';
      x.lineWidth = s.stroke || 2.5;
      x.lineCap='round'; x.lineJoin='round';
      if (s.type === 'pen'){
        if (!s.points || s.points.length<2){ x.restore(); return; }
        x.beginPath(); x.moveTo(s.points[0].x, s.points[0].y);
        for (var i=1;i<s.points.length;i++) x.lineTo(s.points[i].x, s.points[i].y);
        x.stroke();
      } else if (s.type === 'line'){
        ...
        x.beginPath();
        x.moveTo(s.x2, s.y2);
        x.lineTo(s.x2 - ah*Math.cos(angle-Math.PI/6), s.y2 - ah*Math.sin(angle-Math.PI/6));
        x.lineTo(s.x2 - ah*Math.cos(angle+Math.PI/6), s.y2 - ah*Math.sin(angle+Math.PI/6));
        x.closePath(); x.fill();
      } else if (s.type === 'rect'){
        x.strokeRect(Math.min(s.x1,s.x2), Math.min(s.y1,s.y2), Math.abs(s.x2-s.x1), Math.abs(s.y2-s.y1));
        ...
        x.font = (14 + s.stroke*1.5) + 'px "Kalam", Georgia, serif';
        x.textBaseline = 'top';
        x.fillText(s.text || '', s.x, s.y);
      } else if (s.type === 'number'){
        var r = 14 + s.stroke;
        x.beginPath(); x.arc(s.x,s.y,r,0,Math.PI*2); x.fillStyle = s.color; x.fill();
        x.fillStyle = '#fff'; x.font = 'bold ' + (12+s.stroke) + 'px sans-serif';
        x.textAlign='center'; x.textBaseline='middle';
        x.fillText(String(s.n), s.x, s.y);
      }
      x.restore();
    });
    return c;
  ...
  // Public: inline placeholder card for the answer sheet
  window.renderBlockDiagram = function(block){
    var card = document.createElement('div');
    card.className = 'ab-diagram-placeholder';
    var hasContent = (block.dgmHistory && block.dgmHistory.length > 0);

    function refresh(){
      card.innerHTML = '';
      var hasC = (block.dgmHistory && block.dgmHistory.length > 0);
      if (hasC){
        // Thumbnail preview
        var thumb = renderDiagramToCanvasOffscreen(block.dgmHistory, 1000, 420);
        var img = document.createElement('img');
        img.className = 'ab-dgm-thumb';
        img.src = thumb.toDataURL('image/png');
        img.alt = block.caption || 'Diagram';
        card.appendChild(img);

        var meta = document.createElement('div');
        meta.className = 'ab-dgm-meta';
        meta.innerHTML =
          '<span class="ab-dgm-label">🖼 Diagram</span>' +
          '<span class="ab-dgm-caption-disp">' + (block.caption ? escapeHtml(block.caption) : '') + '</span>' +
          '<button type="button" class="ab-dgm-edit-btn">✏️ Tap to edit</button>';
        card.appendChild(meta);
      } else {
        var empty = document.createElement('div');
        empty.className = 'ab-dgm-empty';
        empty.innerHTML =
          '<div class="ab-dgm-empty-icon">🖼</div>' +
          '<div class="ab-dgm-empty-title">Tap to draw your diagram</div>' +
          '<div class="ab-dgm-empty-sub">Pen, shapes, arrows, labels — full canvas</div>';
    ...
    card.onclick = function(){
      openDiagramEditor(block, function(){ refresh(); });
    };
  ...
  // Opens the full-screen drawing modal. Calls onClose() when user
  // taps "Place on Sheet" or "Done" — at that point block.dgmHistory
  // and block.caption are already saved.
  function openDiagramEditor(block, onClose){
    // Remove any existing overlay
    var existing = document.getElementById('dgmEditorOverlay');
    if (existing) existing.remove();

    var overlay = document.createElement('div');
    overlay.id = 'dgmEditorOverlay';
    overlay.className = 'dgm-editor-overlay';

    var modal = document.createElement('div');
    modal.className = 'dgm-editor-modal';

    // Header
    var header = document.createElement('div');
    header.className = 'dgm-editor-header';
    header.innerHTML =
      '<div class="dgm-editor-title">🖼 Diagram Editor</div>' +
      '<div class="dgm-editor-header-actions">' +
        '<button type="button" class="dgm-editor-cancel">Cancel</button>' +
        '<button type="button" class="dgm-editor-place">✓ Place on Sheet</button>' +
      '</div>';
    modal.appendChild(header);

    // Body: this is the same body the old renderBlockDiagram built —
    // we just call it here.
    var body = buildDiagramBody(block);
    body.classList.add('dgm-editor-body');
    ...
    header.querySelector('.dgm-editor-place').onclick = close;
    header.querySelector('.dgm-editor-cancel').onclick = close; // history is autosaved on each stroke
  }

  // The actual editor body (tools + canvas). Originally this was
  // `renderBlockDiagram` — it's been moved here so the public function
  // can render a placeholder.
  function buildDiagramBody(block){
