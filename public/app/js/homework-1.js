...
function goTo(id){
  ...
  // When entering the Parent Hub, default to the Dashboard tab and
  // refresh its content so it reflects the latest child data.
  if(id==='pg-parent'){
    setTimeout(function(){
      if (typeof phTab === 'function') phTab('dashboard');
    }, 30);
  }
}

// ════════════════════ PARENT HUB ROUTING ════════════════════
// Wherever "Parent Hub" / "Parents" is clicked we go straight to the
// dashboard view of #pg-parent. Admins + parents see the full hub.
// Unauthenticated users still land here too — the hub itself shows
// "add a child to begin" so they can preview. This keeps presentation
// simple and predictable.
function openParentsRoute(){
  try { closeMob && closeMob(); } catch(e){}
  try { closeSidebar && closeSidebar(); } catch(e){}
  goTo('pg-parent');
  // phTab('dashboard') is called inside goTo when id==='pg-parent'
}
window.openParentsRoute = openParentsRoute;
...
(repeat the same format for all 5 questions, each followed by its own --- line)`;
  ...
  try{
    ...
    // Show a brief hint pointing at the Teach Me Voice button so students
    // discover the feature on their first lesson view.
    setTimeout(function(){
      try { if (window.VoiceTeacher && window.VoiceTeacher.showHint) window.VoiceTeacher.showHint(); } catch(e){}
    }, 1200);
...
function renderRichLesson(lesson, s, topicTitle, term, idx){
  ...
  <!-- TEACH ME VOICE + ASK LESSON TEACHER AI BUTTONS -->
  <div class="lt-action-btns">
    <button class="vt-teach-btn" id="vtTeachBtn" onclick="(function(){
      if(window.VoiceTeacher){
        var prof = window._LT_LAST_PROFILE;
        var sName = (prof && prof.display_name) ? prof.display_name.split(' ')[0] : '';
        var body = document.getElementById('lessonBody');
        var lessonContent = body ? body.innerHTML : '';
        window.VoiceTeacher.teach({
          title: '${topicTitle.replace(/'/g,"\\'")}',
          subject: '${s.name}',
      ...
      <span class="vt-teach-icon">🔊</span>
      <span>Teach Me Voice</span>
      <span class="vt-teach-live">SUMMARY</span>
    </button>
    <button class="vt-listen-btn" id="vtExplainBtn" onclick="openExplainToMe('${topicTitle.replace(/'/g,"\\'")}','${s.name}')">
      <span class="vt-listen-icon">👨🏿‍🏫</span>
      <span>Ask Lesson Teacher AI</span>
    </button>
  </div>`;
...
async function reteachTopic(subjKey, term, idx){
  ...
  try{
    ...
    if(reply){ chatHistory.push({role:'assistant',content:reply}); addChatMsg('lt',reply); /* text-only — no voice in chat */ }
...
async function fetchTutorReply(s, topicTitle){
  ...
  if (canStream){
    try {
      ...
      if (raw){
        ...
        // Text-only reply — no voice speaking in AI chatbot.
        // Voice is reserved for "Teach Me Voice" button (lesson summary).
      }
      return;
    } catch(streamErr){
  ...
  try{
    ...
    if(raw){
      ...
      // Text-only reply — no voice speaking in chat. Voice is for Teach Me only.
    }
  }catch(e){
...
// Tracks IDs/fingerprints of questions a student has already answered
// so they don't see the same WAEC question twice. Stored per
// (board, subject) so different exams don't share blacklists.
// ════════════════════════════════════════════════════════════════
function _seenKey(board, subj){
  return 'lt_seen_qs_' + (board||'?') + '_' + (subj||'?');
}
function _qFingerprint(q){
  // Stable hash of the question text (first 80 chars). Used so AI-generated
  // questions that share text with hardcoded ones are recognised.
  var t = (q && (q.q || q.question || q.text) || '').toString().toLowerCase()
    .replace(/\s+/g,' ').replace(/[^a-z0-9 ]/g,'').trim().slice(0,80);
  return t || (q && q.id) || '';
}
function getSeenQuestions(board, subj){
  try {
    var raw = localStorage.getItem(_seenKey(board, subj));
    return raw ? JSON.parse(raw) : [];
  } catch(e){ return []; }
}
function markQuestionsAsSeen(board, subj, questions){
  if (!questions || !questions.length) return;
  try {
    var seen = getSeenQuestions(board, subj);
    var seenSet = {}; seen.forEach(function(f){ seenSet[f] = 1; });
    questions.forEach(function(q){
      var f = _qFingerprint(q);
      if (f && !seenSet[f]){ seen.push(f); seenSet[f] = 1; }
    });
    // Cap at 500 entries to avoid unbounded growth
    if (seen.length > 500) seen = seen.slice(-500);
    localStorage.setItem(_seenKey(board, subj), JSON.stringify(seen));
  } catch(e){}
}
function filterUnseenQuestions(board, subj, questions){
  if (!questions || !questions.length) return [];
  var seen = getSeenQuestions(board, subj);
  if (!seen.length) return questions;
  var seenSet = {}; seen.forEach(function(f){ seenSet[f] = 1; });
...
window.markQuestionsAsSeen = markQuestionsAsSeen;
window.getSeenQuestions = getSeenQuestions;
...
function getRealBankQuestions(subjKey,n){
  ...
  var normalised = qs.map(normaliseQuestion);
  // Filter out questions this student has already answered in this board+subject.
  // If filtering leaves too few (< requested count), return what we have so the
  // session-launcher can request AI top-ups for the remainder.
  var unseen = filterUnseenQuestions(currentExam, subjKey, normalised);
  return unseen;
}
...
// Tracks the year filter selected on the exam rules screen.
// 'any' = no filter; otherwise '2024', '2023', etc.
var _essayYearFilter = 'any';

function setEssayYear(yr, btn){
  _essayYearFilter = yr;
  document.querySelectorAll('.erc-year-chip').forEach(function(b){b.classList.remove('active');});
  if (btn) btn.classList.add('active');
  // Reload topics with the new year filter
  if (typeof loadEssayTopics === 'function'){
    loadEssayTopics(!!window._essayExamContext);
  }
}
window.setEssayYear = setEssayYear;

// Loads theory questions for the current exam. AI-FIRST: tries to generate
// a fresh, syllabus-aligned, copyright-safe paper. Falls back to the
// legacy hardcoded ESSAY_TOPICS_BY_SUBJ only if AI is unavailable.
function loadEssayTopics(fromExam) {
  var grid=document.getElementById('essayTopicGrid');
  if(!grid) return;
  var boardKey = 'waec', subjKey = 'default', subjName = '';
  var ex = 'WAEC';
  if(fromExam && window._essayExamContext){
    var ctx=window._essayExamContext;
    ex = (ctx.exam || 'waec').toUpperCase();
    boardKey = (ctx.exam || 'waec').toLowerCase();
    var rawKey=ctx.key||'default';
    subjKey=rawKey.replace(/-[a-z0-9]+$/,'');
    subjName = ctx.subj || subjKey;
  } else {
  ...
  // Show loading state while paper is fetched
  grid.innerHTML = '<div class="erc-topic-loading" style="padding:30px 20px;text-align:center;color:rgba(255,255,255,.7);font-size:.88rem;">'
    + '<div style="font-size:2.2rem;margin-bottom:10px;animation:pulse 1.4s ease-in-out infinite">📝</div>'
    + '<div style="font-weight:800;color:#fbbf24;margin-bottom:6px">Preparing your ' + ex + ' ' + subjName + ' paper...</div>'
    + '<div style="font-size:.76rem;opacity:.7">Loading fresh questions from the NERDC syllabus.</div>'
    + '</div>';

  function renderTopics(topics, isAI){
    ...
    grid.innerHTML = topics.map(function(t,i){
      ...
      return '<button class="erc-topic-btn" onclick="pickEssayTopic('+i+',this)">'
        + '<div class="erc-topic-type" style="display:flex;align-items:center">'+ t.type + badge + '</div>'
        + '<div class="erc-topic-text">' + (t.q || '') + '</div></button>';
    }).join('');
  }

  // Try AI generation FIRST (preferred — fresh, copyright-safe, syllabus-aligned)
  if (window.ExamGen && typeof window.ExamGen.generate === 'function') {
    window.ExamGen.generate({
      board: ex,
      subject: subjKey,
      subjectName: subjName,
      yearStyle: _essayYearFilter !== 'any' ? _essayYearFilter : null
    }).then(function(aiTopics){
      if (aiTopics && aiTopics.length){
        // Tag with exam label so card UI shows it
        aiTopics = aiTopics.map(function(t){
          return Object.assign({}, t, {
            type: '[' + ex + '] ' + (t.type || 'Question'),
            _exam: ex,
            _subj: subjName
          });
        });
        // Filter unseen
        try {
          if (typeof filterUnseenQuestions === 'function'){
            var unseen = filterUnseenQuestions(boardKey, subjKey + '_essay', aiTopics);
            if (unseen.length >= 2) aiTopics = unseen;
          }
        } catch(e){}
        renderTopics(aiTopics, true);
      } else {
        renderTopics(fallbackTopics(), false);
      }
    }).catch(function(err){
      console.warn('[loadEssayTopics] AI generation failed, falling back to local', err);
      try { if (typeof showLocalFallbackToast === 'function') showLocalFallbackToast(); } catch(e){}
      renderTopics(fallbackTopics(), false);
    });
  } else {
    renderTopics(fallbackTopics(), false);
  ...
  function fallbackTopics(){
    ...
    try {
      if (typeof filterUnseenQuestions === 'function'){
        var unseen = filterUnseenQuestions(boardKey, subjKey + '_essay', topics);
        if (unseen.length >= 2) topics = unseen;
      }
    } catch(e){}
    return topics;
  }
...
function startEssayWriting() {
  ...
  // Hide floating FABs during exam
  document.body.classList.add('in-exam');

  // Auto-collapse question panel on mobile — maximize writing space
  if(window.innerWidth<=700){
    theoryQuestionPanelVisible=false;
    var col=document.querySelector('.theory-question-col');
    if(col){col.classList.add('mob-collapsed');col.classList.remove('mob-open');col.style.display='';}
    var fab=document.getElementById('mobQFab');
    if(fab) fab.style.display='flex';
    var tbQ=document.getElementById('tbQuestion');
    if(tbQ) tbQ.classList.remove('on');
  }
...
// ════════════════════════════════════════════════════════════════
// SUBJECT-SPECIFIC ADD-BLOCK BAR
// Defines which answer-block types appear in the add-bar per subject.
// Each subject sees ONLY the blocks that WAEC marks for that paper.
// ════════════════════════════════════════════════════════════════
var BLOCK_DEFS = {
  free:        { ico:'✍️', label:'Write',      type:'free' },
  point:       { ico:'•',  label:'Points',     type:'point' },
  steps:       { ico:'🔢', label:'Steps',      type:'steps' },
  def:         { ico:'📖', label:'Definition', type:'def' },
  exp:         { ico:'💡', label:'Explanation',type:'exp' },
  eg:          { ico:'✏️', label:'Example',    type:'eg' },
  table:       { ico:'⊞',  label:'Table',     type:'table' },
  equation:    { ico:'⚡', label:'Equation',   type:'equation' },
  diagram:     { ico:'🖼', label:'Diagram',    type:'diagram' },
  quote:       { ico:'📜', label:'Quote',      type:'quote' },
  ledger:      { ico:'📒', label:'Ledger',     type:'ledger' },
  timeline:    { ico:'📅', label:'Timeline',   type:'timeline' },
  extrasheet:  { ico:'📄', label:'Extra Sheet',type:'extrasheet' }
};

// Map of subject-key → ordered list of available blocks
var BLOCKS_BY_SUBJECT = {
  // English / Literature — writing-heavy
  eng:  ['free','point','def','exp','eg','quote','extrasheet'],
  lit:  ['free','point','quote','exp','extrasheet'],
  // Maths / Further Maths — workings + diagrams
  mth:  ['free','steps','equation','diagram','table','extrasheet'],
  fmth: ['free','steps','equation','diagram','table','extrasheet'],
  // Sciences — diagrams + equations + tables
  phy:  ['free','steps','equation','diagram','table','extrasheet'],
  chm:  ['free','steps','equation','diagram','table','extrasheet'],
  bio:  ['free','point','def','exp','diagram','table','extrasheet'],
  bsc:  ['free','point','steps','diagram','table','extrasheet'],
  // Geography — diagrams (maps, climate), tables, structured
  geo:  ['free','point','diagram','table','extrasheet'],
  // Economics — graphs (as diagrams), tables, structured
  eco:  ['free','point','def','exp','table','diagram','extrasheet'],
  // History — timelines, quotes, structured
  his:  ['free','point','timeline','quote','extrasheet'],
  // Religious Studies — quotes (scripture), structured
  crs:  ['free','point','quote','exp','extrasheet'],
  irs:  ['free','point','quote','exp','extrasheet'],
  // Government / Civic — definitions, points, quotes
  gov:  ['free','point','def','exp','quote','extrasheet'],
  govt: ['free','point','def','exp','quote','extrasheet'],
  // Agric Science — diagrams, tables, calculations
  agr:  ['free','point','diagram','table','steps','extrasheet'],
  // Accounting / Commerce — ledgers, financial tables, steps
  acc:  ['free','ledger','table','steps','extrasheet'],
  com:  ['free','point','table','extrasheet'],
  // Computer Studies — diagrams, flowcharts, tables
  cmp:  ['free','point','steps','diagram','table','extrasheet'],
  // PHE — structured + diagrams (anatomy)
  phe:  ['free','point','def','exp','diagram','extrasheet'],
  // Social Studies / General Paper
  sst:  ['free','point','def','exp','diagram','table','extrasheet'],
  gen:  ['free','point','def','exp','extrasheet'],
  sci:  ['free','point','steps','diagram','table','extrasheet'],
  // Languages
  fre:  ['free','point','quote','extrasheet'],
  // Default fallback
  default: ['free','point','def','exp','table','diagram','extrasheet']
};

function getSubjectBlockKeys(){
  var ctx = window._essayExamContext;
  var subjKey = 'default';
  if (ctx && ctx.key) {
    subjKey = ctx.key.replace(/-[a-z0-9]+$/, '');
  } else if (typeof currentSubject !== 'undefined' && currentSubject) {
    subjKey = currentSubject.replace(/-[a-z0-9]+$/, '');
  }
  return BLOCKS_BY_SUBJECT[subjKey] || BLOCKS_BY_SUBJECT.default;
}

function renderAddBlockBar(){
  var bar = document.getElementById('tapAddBar');
  if (!bar) return;
  // Preserve the label, replace buttons
  bar.innerHTML = '<span class="tap-add-lbl">Add to sheet:</span>';
  var keys = getSubjectBlockKeys();
  keys.forEach(function(key){
    var def = BLOCK_DEFS[key];
    if (!def) return;
    var btn = document.createElement('button');
    btn.className = 'tap-add-btn';
    if (key === 'diagram') btn.className += ' tap-add-btn-diagram';
    if (key === 'extrasheet') btn.className += ' tap-add-btn-extra';
    btn.innerHTML = def.ico + ' ' + def.label;
    btn.onclick = (function(t){ return function(){ addBlock(t); }; })(def.type);
    bar.appendChild(btn);
  });
}

// Builds a sticky "Jump to block" chip strip above the answer pane.
// Only shown when there are 3+ blocks — helps students navigate long sheets.
function renderBlockNav(answers, content){
  var navEl = document.getElementById('tapBlockNav');
  var listEl = document.getElementById('tapBlockNavList');
  if (!navEl || !listEl) return;
  var contentBlocks = (answers || []).filter(function(b){ return b.type !== 'extrasheet'; });
  if (contentBlocks.length < 3){ navEl.style.display = 'none'; return; }

  var icons = {free:'✍️',def:'📖',exp:'💡',eg:'✏️',point:'•',steps:'🔢',table:'⊞',equation:'⚡',diagram:'🖼',quote:'📜',ledger:'📒',timeline:'📅'};
  var labels = {free:'Write',def:'Def',exp:'Exp',eg:'Eg',point:'Points',steps:'Steps',table:'Table',equation:'Eq',diagram:'Diagram',quote:'Quote',ledger:'Ledger',timeline:'Timeline'};

  listEl.innerHTML = '';
  var num = 0;
  (answers || []).forEach(function(b){
    if (b.type === 'extrasheet') {
      var sep = document.createElement('span');
      sep.className = 'tap-block-nav-sheet';
      sep.textContent = '📄';
    ...
    chip.type = 'button';
    chip.className = 'tap-block-nav-chip';
    chip.innerHTML = '<span class="tap-block-nav-num">'+num+'</span>'
      + '<span class="tap-block-nav-ico">'+(icons[b.type]||'·')+'</span>'
      + '<span class="tap-block-nav-name">'+(labels[b.type]||'')+'</span>';
    chip.onclick = function(){
      var el = document.querySelector('[data-bid="'+b.id+'"]');
      if (el && content) {
        try { el.scrollIntoView({behavior:'smooth', block:'start'}); } catch(e){}
        el.classList.add('answer-block-flash');
        setTimeout(function(){ el.classList.remove('answer-block-flash'); }, 1200);
      }
    };
...
function updateQuestionBanner(){
  ...
  if (!q) return;
  var textEl = document.getElementById('tapQBannerText');
  var partEl = document.getElementById('tapQBannerPart');
  if (textEl) textEl.textContent = 'Question ' + (theoryCurrentQ+1) + ' of ' + theoryQuestions.length;
  if (partEl) {
    var partIdx = q.activePart || 0;
    var part = q.parts[partIdx];
    partEl.textContent = part ? ('Part ' + part.label) : '';
  }
}

// Toggle the answer sheet between normal and fullscreen modes.
// In fullscreen, the answer-pane fills the entire viewport so students have
// max writing space and clean scroll behavior.
function toggleSheetFullscreen(){
  var isFs = document.body.classList.toggle('sheet-fullscreen');
  var btn = document.getElementById('tapExpandBtn');
  if (btn) {
    btn.textContent = isFs ? '⤓' : '⛶';
    btn.title = isFs ? 'Exit fullscreen' : 'Expand answer sheet to full screen';
  }
  // When entering fullscreen, scroll the answer area to the top
  if (isFs) {
    var tap = document.getElementById('tapContent');
    if (tap) tap.scrollTop = 0;
  }
}
...
function renderCurrentQuestion() {
  ...
  // Render subject-specific add-block bar
  renderAddBlockBar();

  // Scroll answer sheet to top when switching questions
  var tapContent = document.getElementById('tapContent');
  if (tapContent) tapContent.scrollTop = 0;
...
function switchPart(idx) {
  ...
  if (typeof updateQuestionBanner === 'function') updateQuestionBanner();
  renderPartTabs(q);
  renderAnswerPane(q, idx);
  // Scroll answer sheet to top when switching parts
  var tapContent = document.getElementById('tapContent');
  if (tapContent) tapContent.scrollTop = 0;
...
function renderAnswerPane(q, partIdx) {
  ...
  // Number each block for clear organization (skip extrasheet which is a page break)
  var blockNum = 0;
  answers.forEach(function(block){
    if (block.type !== 'extrasheet') blockNum++;
    block._displayNum = blockNum;
    content.appendChild(renderBlock(block));
  });

  // Render the jump-to-block navigation if there are 3+ content blocks
  renderBlockNav(answers, content);

  updateTheoryWordCount();
  // Auto-add a free-write block if the part is completely empty, OR if all
  // existing blocks are non-writing types (extrasheet / diagram only)
  var hasWriting = answers.some(function(b){
    return b && b.type !== 'extrasheet' && b.type !== 'diagram';
  });
  if(answers.length===0 || !hasWriting) addBlock('free');
}
...
function renderBlock(block) {
  ...
  // Block number label (e.g. "Block 1") — helps long sheets stay organized
  var numLabel = document.createElement('span');
  numLabel.className = 'ab-num';
  if (block.type !== 'extrasheet' && block._displayNum){
    numLabel.textContent = 'BLOCK ' + block._displayNum;
  }
  ...
  // Action buttons container (Expand + Delete)
  var actions = document.createElement('div');
  actions.className = 'ab-actions';

  // EXPAND button — opens block in a focused full-screen editor
  // (Diagram, Table, ExtraSheet already have their own flows, skip them)
  var TEXT_BLOCK_TYPES = {free:1,def:1,exp:1,eg:1,point:1,steps:1,equation:1,quote:1};
  if (TEXT_BLOCK_TYPES[block.type]){
    var expandBtn = document.createElement('button');
    expandBtn.className = 'ab-expand';
    expandBtn.innerHTML = '<span class="ab-expand-icon">⤢</span><span class="ab-expand-label">Expand</span>';
    expandBtn.title = 'Expand this block to full screen for easier writing';
    expandBtn.onclick = function(e){
      if (e) e.stopPropagation();
      openTextBlockEditor(block);
    };
    actions.appendChild(expandBtn);
  }

  // DELETE button — bigger, red, very visible
  ...
  del.onclick=function(e){
    if (e) e.stopPropagation();
    if (confirm('Remove this '+ (labels[block.type] || 'block') +'?')) {
      removeBlock(block.id);
    }
  };
  actions.appendChild(del);

  // Title bundle (number + badge)
  var titleBundle = document.createElement('div');
  titleBundle.className = 'ab-title-bundle';
  if (numLabel.textContent) titleBundle.appendChild(numLabel);
  titleBundle.appendChild(badge);

  hdr.appendChild(titleBundle);
  ...
  if(block.type==='point') {
    ...
    // Table block — placeholder + modal editor (same pattern as Diagram)
    if(!block.cols||block.cols.length===0) block.cols=['Feature','Column A','Column B'];
    if(!block.rows||block.rows.length===0) block.rows=[['','',''],['','','']];
    var tablePlaceholder = renderTablePlaceholder(block);
    wrap.appendChild(tablePlaceholder);
  ...
  } else if(block.type==='extrasheet') {
    // True page break — content above goes to "Sheet 1", content below to
    // "Sheet 2", etc. The break shows a styled "End of Sheet N • Continued
    // on Sheet N+1" indicator. Block is positioned by index in the answer
    // array; we compute its sheet number on render.
    var es = document.createElement('div');
    es.className = 'ab-extra-sheet';
    // Determine which sheet this break ends (count preceding extrasheets +1)
    var sheetNum = 1;
    var q2 = theoryQuestions[theoryCurrentQ];
    if (q2){
      var partAns = q2.answers[q2.activePart||0] || [];
      for (var i2 = 0; i2 < partAns.length; i2++){
        if (partAns[i2].id === block.id) break;
        if (partAns[i2].type === 'extrasheet') sheetNum++;
      }
    }
    es.innerHTML =
      '<div class="ab-extra-sheet-rule"></div>' +
      '<div class="ab-extra-sheet-stamp">' +
        '<span class="ab-extra-sheet-icon">📄</span>' +
        '<span class="ab-extra-sheet-text">' +
          '<strong>End of Sheet ' + sheetNum + '</strong>' +
          '<small>Continued on Sheet ' + (sheetNum+1) + '</small>' +
        '</span>' +
      '</div>' +
      '<div class="ab-extra-sheet-rule"></div>';
    // Delete button
    var delBtn = document.createElement('button');
    delBtn.className = 'ab-extra-sheet-del';
    delBtn.textContent = '✕';
    delBtn.title = 'Remove this page break';
    delBtn.onclick = function(e){
      if (e) e.stopPropagation();
      removeBlock(block.id);
    };
    es.appendChild(delBtn);
    wrap.appendChild(es);
    // Hide the standard ab-header for this block (it's a page break, not content)
    wrap.classList.add('ab-no-header');
  } else {
...
// ════════════════════════════════════════════════════════════════
// TABLE BLOCK — placeholder + full-screen modal editor
// (mirrors the Diagram block pattern)
// ════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════
// UNIVERSAL TEXT-BLOCK EDITOR MODAL
// Opens any text-based block (free, def, exp, eg, point, steps, quote,
// equation) in a focused full-screen editor with a giant textarea, big
// "✓ Save & Place" button, and clear cancel.
// ════════════════════════════════════════════════════════════════
function openTextBlockEditor(block){
  var existing = document.getElementById('textBlockEditorOverlay');
  if (existing) existing.remove();

  var LABELS = {
    free:'Free Write', def:'Definition', exp:'Explanation', eg:'Example',
    point:'Points', steps:'Steps', equation:'Equation', quote:'Quote/Evidence'
  };
  var PLACEHOLDERS = {
    free:'Write your answer here…',
    def:'Define the term or concept clearly…',
    exp:'Explain in detail…',
    eg:'Give a specific example…',
    point:'Type each point on a new line…',
    steps:'Step 1: …\nStep 2: …\nStep 3: …',
    equation:'Type your equation, e.g.\n2x + 3 = 7\n2x = 4\nx = 2',
    quote:'"Quote or evidence text"\n— Source / author'
  };

  var overlay = document.createElement('div');
  overlay.id = 'textBlockEditorOverlay';
  overlay.className = 'tbe-overlay';
  overlay.innerHTML =
    '<div class="tbe-modal">' +
      '<div class="tbe-header">' +
        '<div class="tbe-title">📝 ' + (LABELS[block.type] || 'Write') + '</div>' +
        '<div class="tbe-actions">' +
          '<button class="tbe-cancel" type="button">Cancel</button>' +
          '<button class="tbe-save" type="button">✓ Save &amp; Place</button>' +
        '</div>' +
      '</div>' +
      '<div class="tbe-body">' +
        '<textarea class="tbe-textarea" placeholder="' + (PLACEHOLDERS[block.type] || 'Write here…').replace(/"/g,'&quot;') + '"></textarea>' +
      '</div>' +
      '<div class="tbe-footer">' +
        '<span class="tbe-wc" id="tbeWordCount">0 words</span>' +
        '<span class="tbe-hint">Tap outside or Cancel to discard changes</span>' +
      '</div>' +
    '</div>';

  document.body.appendChild(overlay);

  var ta = overlay.querySelector('.tbe-textarea');
  // Pre-fill with current content
  if (block.type === 'point' && block.points) {
    ta.value = block.points.join('\n');
  } else if (block.type === 'steps' && block.steps) {
    ta.value = block.steps.join('\n');
  } else {
    ta.value = block.content || '';
  }

  // Block clipboard violations (paste/copy/cut/contextmenu)
  ta.onpaste = function(e){ e.preventDefault(); if (typeof logEssayViolation === 'function') logEssayViolation('Paste'); };
  ta.oncopy = function(e){ e.preventDefault(); if (typeof logEssayViolation === 'function') logEssayViolation('Copy'); };
  ta.oncut = function(e){ e.preventDefault(); if (typeof logEssayViolation === 'function') logEssayViolation('Cut'); };
  ta.oncontextmenu = function(e){ e.preventDefault(); };

  function updateWC(){
    var n = (ta.value || '').trim().split(/\s+/).filter(Boolean).length;
    overlay.querySelector('#tbeWordCount').textContent = n + ' words';
  }
  ta.oninput = updateWC;
  updateWC();
  ...
  function save(){
    var val = ta.value;
    if (block.type === 'point') {
      block.points = val.split('\n').map(function(s){return s.trim();}).filter(function(s){return s.length > 0;});
      if (block.points.length === 0) block.points = [''];
    } else if (block.type === 'steps') {
      block.steps = val.split('\n').filter(function(s){return s.trim().length > 0;});
      if (block.steps.length === 0) block.steps = ['Step 1: '];
    } else {
      block.content = val;
    }
    saveCurrentAnswer();
    // Re-render this block in place
    var existingEl = document.querySelector('[data-bid="'+block.id+'"]');
    if (existingEl) {
      var newEl = renderBlock(block);
  ...
  overlay.querySelector('.tbe-cancel').onclick = close;
  overlay.querySelector('.tbe-save').onclick = save;
  // Esc closes
  overlay.tabIndex = -1;
  overlay.focus();
...
function renderTablePlaceholder(block){
  var card = document.createElement('div');
  card.className = 'ab-table-placeholder';

  function refresh(){
    card.innerHTML = '';
    var hasContent = (block.rows||[]).some(function(r){
      return (r||[]).some(function(c){ return c && String(c).trim(); });
    });
    if (hasContent || (block.cols && block.cols.length && (block.cols[0]!=='Feature' || block.cols.length>3))) {
      // Mini-preview of the table
      var prev = document.createElement('table');
      prev.className = 'ab-table-mini';
      var thead = document.createElement('thead'); var htr = document.createElement('tr');
      (block.cols||[]).forEach(function(c){
        var th = document.createElement('th'); th.textContent = c || ''; htr.appendChild(th);
      });
      thead.appendChild(htr); prev.appendChild(thead);
      var tbody = document.createElement('tbody');
      (block.rows||[]).slice(0, 4).forEach(function(r){
        var tr = document.createElement('tr');
        (block.cols||[]).forEach(function(_, i){
          var td = document.createElement('td');
          td.textContent = (r && r[i]) || '';
          tr.appendChild(td);
        });
        tbody.appendChild(tr);
      });
      prev.appendChild(tbody);
      card.appendChild(prev);

      var meta = document.createElement('div');
      meta.className = 'ab-table-meta';
      var rowCount = (block.rows||[]).length;
      var colCount = (block.cols||[]).length;
      meta.innerHTML =
        '<span class="ab-table-meta-label">⊞ Table</span>' +
        '<span class="ab-table-meta-size">' + rowCount + ' rows × ' + colCount + ' cols</span>' +
        '<button type="button" class="ab-table-edit-btn">✏️ Tap to edit</button>';
      card.appendChild(meta);
    } else {
      var empty = document.createElement('div');
      empty.className = 'ab-table-empty';
      empty.innerHTML =
        '<div class="ab-table-empty-icon">⊞</div>' +
        '<div class="ab-table-empty-title">Tap to build a table</div>' +
        '<div class="ab-table-empty-sub">Rows, columns, headers — opens in dedicated editor</div>';
  ...
  card.onclick = function(){
    openTableEditor(block, function(){ refresh(); saveCurrentAnswer(); });
  };
  return card;
}

function openTableEditor(block, onClose){
  var existing = document.getElementById('tableEditorOverlay');
  if (existing) existing.remove();

  var overlay = document.createElement('div');
  overlay.id = 'tableEditorOverlay';
  overlay.className = 'tbl-editor-overlay';

  var modal = document.createElement('div');
  modal.className = 'tbl-editor-modal';

  var header = document.createElement('div');
  header.className = 'tbl-editor-header';
  header.innerHTML =
    '<div class="tbl-editor-title">⊞ Table Editor</div>' +
    '<div class="tbl-editor-actions">' +
      '<button type="button" class="tbl-editor-cancel">Cancel</button>' +
      '<button type="button" class="tbl-editor-place">✓ Place on Sheet</button>' +
    '</div>';
  modal.appendChild(header);

  var body = document.createElement('div');
  body.className = 'tbl-editor-body';
  modal.appendChild(body);

  function rebuildBody(){
    body.innerHTML = '';
    var twrap = document.createElement('div'); twrap.className = 'ab-table-wrap';
    twrap.appendChild(buildTable(block));
    var btnRow = document.createElement('div'); btnRow.className = 'tbl-editor-btnrow';
    var addRow = document.createElement('button');
    addRow.className = 'ab-table-add-row';
    addRow.textContent = '+ Add Row';
    addRow.onclick = function(){ block.rows.push(block.cols.map(function(){return '';})); rebuildBody(); };
    var addCol = document.createElement('button');
    addCol.className = 'ab-table-add-col';
    addCol.textContent = '+ Column';
    addCol.onclick = function(){ block.cols.push('Column '+(block.cols.length+1)); block.rows.forEach(function(r){r.push('');}); rebuildBody(); };
    btnRow.appendChild(addRow); btnRow.appendChild(addCol);
    twrap.appendChild(btnRow);
  ...
  header.querySelector('.tbl-editor-place').onclick = close;
  header.querySelector('.tbl-editor-cancel').onclick = close;
}
...
function addBlock(type) {
  ...
  // For Extra Sheet, also auto-add a fresh Free Write block so user has
  // a new sheet to write on right away.
  if (type === 'extrasheet') {
    q.answers[partIdx].push({id:nextBlockId(),type:'free',content:''});
  }
  // Re-render the whole pane so block numbers + nav stay in sync
  saveCurrentAnswer();
  renderAnswerPane(q, partIdx);
  // Focus + scroll to the new block
  setTimeout(function(){
    var content = document.getElementById('tapContent');
    var newEl = content && content.querySelector('[data-bid="'+block.id+'"]');
    if (newEl){
      try { newEl.scrollIntoView({behavior:'smooth', block:'center'}); } catch(e){}
      if (type !== 'diagram' && type !== 'extrasheet'){
        var ta = newEl.querySelector('textarea');
...
function removeBlock(bid) {
  ...
  // Re-render whole pane so block numbers + nav refresh
  renderAnswerPane(q, partIdx);
  updateTheoryWordCount();
}
...
// Question panel now opens as a proper full-screen MODAL overlay with
// navigation (prev/next), status badges (Submitted/Draft/Skipped), and a
// clear close button. This guarantees the question is always visible
// regardless of CSS caching or layout issues.
function toggleQuestionPanel() {
  var existing = document.getElementById('qpModal');
  if (existing){ existing.remove(); document.body.classList.remove('q-modal-open'); return; }
  showQuestionModal();
}
...
function showQuestionModal(){
  ...
  modal.innerHTML = buildQuestionModalHTML(q);
  document.body.appendChild(modal);
  document.body.classList.add('q-modal-open');

  // Wire buttons
  modal.querySelector('.qp-close').onclick = function(){
    modal.remove();
    document.body.classList.remove('q-modal-open');
  };
  modal.querySelector('.qp-backdrop').onclick = function(){
    modal.remove();
    document.body.classList.remove('q-modal-open');
  };
  var prevBtn = modal.querySelector('.qp-prev');
  var nextBtn = modal.querySelector('.qp-next');
  ...
  // Wire question-number jump buttons
  modal.querySelectorAll('.qp-qjump').forEach(function(b){
    b.onclick = function(){
      var idx = parseInt(b.dataset.idx, 10);
      if (idx !== theoryCurrentQ){
        saveCurrentAnswer();
        theoryCurrentQ = idx;
        renderCurrentQuestion();
      }
...
function buildQuestionModalHTML(q){
  var partsHTML = '';
  if (q.intro) partsHTML += '<div class="qp-intro">'+ escapeHtmlForQ(q.intro) +'</div>';
  if (q.parts && q.parts.length > 0){
    partsHTML += '<div class="qp-parts">';
    q.parts.forEach(function(p){
      partsHTML += '<div class="qp-part"><span class="qp-part-label">'+ escapeHtmlForQ(p.label) +'</span><span class="qp-part-text">'+ escapeHtmlForQ(p.text) +'</span></div>';
    });
    partsHTML += '</div>';
  }

  // Question jump-grid with status badges
  var jumpGrid = '<div class="qp-jump-grid">';
  theoryQuestions.forEach(function(tq, i){
    var status = 'draft';
    var statusLbl = '✏️';
    if (tq.answered) { status = 'submitted'; statusLbl = '✓'; }
    else if (tq.skipped) { status = 'skipped'; statusLbl = '⏭'; }
    else {
      var hasContent = (tq.answers||[]).some(function(part){
        return (part||[]).some(function(b){ return (b.content && b.content.trim()) || (b.points && b.points.some(function(p){return p && p.trim();})); });
      });
      if (!hasContent) { status = 'untouched'; statusLbl = '—'; }
    }
    var active = (i === theoryCurrentQ) ? ' qp-qjump-active' : '';
    jumpGrid += '<button class="qp-qjump qp-qjump-'+status+active+'" data-idx="'+i+'">' +
                '<span class="qp-qjump-num">Q'+(i+1)+'</span>' +
                '<span class="qp-qjump-status">'+statusLbl+'</span>' +
                '</button>';
  });
  jumpGrid += '</div>';

  // Marks chips
  var totalMarks = q.parts.length > 0 ? q.parts.length * 10 : 20;
  var mins = Math.ceil(totalMarks * 0.9);
  var chips = '<span class="qp-chip qp-chip-marks">'+ totalMarks +' marks</span>' +
              '<span class="qp-chip qp-chip-type">'+ escapeHtmlForQ(q.topic && q.topic.type || 'Essay') +'</span>' +
              '<span class="qp-chip qp-chip-time">~'+ mins +' min</span>';

  var prevDisabled = theoryCurrentQ === 0 ? ' disabled' : '';
  var nextDisabled = theoryCurrentQ === theoryQuestions.length - 1 ? ' disabled' : '';
  ...
  return ''
    ...
    +       '<div class="qp-q-label">QUESTION '+ (theoryCurrentQ+1) +' OF '+ theoryQuestions.length +'</div>'
    +       '<div class="qp-chips">'+ chips +'</div>'
    +     '</div>'
    +     '<button class="qp-close" type="button" title="Close — return to writing">✕</button>'
    +   '</div>'
    +   '<div class="qp-body">'
    +     partsHTML
    +   '</div>'
    +   '<div class="qp-nav">'
    +     '<div class="qp-nav-section-label">Jump to question:</div>'
    +     jumpGrid
    +   '</div>'
    +   '<div class="qp-footer">'
    +     '<button class="qp-prev" type="button"'+ prevDisabled +'>← Previous</button>'
    +     '<button class="qp-close-big" type="button" onclick="toggleQuestionPanel()">📝 Continue Writing</button>'
    +     '<button class="qp-next" type="button"'+ nextDisabled +'>Next →</button>'
    +   '</div>'
    + '</div>';
}

function escapeHtmlForQ(s){
  return String(s||'').replace(/[&<>"']/g, function(c){
    return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];
  });
}
// Inject floating Question FAB for mobile (once)
(function(){
  function injectQFab(){
    if(document.getElementById('mobQFab')) return;
    var fab=document.createElement('button');
    fab.id='mobQFab';
    fab.className='mob-q-fab';
    fab.setAttribute('data-no-translate','1');
    fab.innerHTML='📋 Question';
    fab.onclick=function(){ toggleQuestionPanel(); };
    document.body.appendChild(fab);
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',injectQFab);
  else injectQFab();
})();
...
async function submitEssay(){
  ...
  // Mark essay topics as seen so the student doesn't get the same topics again
  try {
    if (typeof markQuestionsAsSeen === 'function'){
      var ctx = window._essayExamContext || {};
      var boardKey = (ctx.exam || currentExam || 'waec').toLowerCase();
      var subjKey  = ((ctx.key || currentExamSubj || '')+'').replace(/-[a-z0-9]+$/, '');
      var topics = (theoryQuestions||[]).map(function(tq){
        return { q: (tq && tq.topic && tq.topic.q) || tq.intro || '' };
      });
      markQuestionsAsSeen(boardKey, subjKey + '_essay', topics);
    }
  } catch(e){ console.warn('essay seen-tracking failed', e); }
...
// ════════════════════════════════════════════════════════════════
// EXPLAIN TO ME — opens the AI chat with a prompt that triggers a
// rich Nigerian-flavored explanation. Adds vivid local examples,
// avoids reading the notes back verbatim.
// ════════════════════════════════════════════════════════════════
function openExplainToMe(topicTitle, subjectName){
  ...
  var prof = window._LT_LAST_PROFILE;
  var firstName = (prof && prof.display_name) ? prof.display_name.split(' ')[0] : 'student';
  var subj = subjectName || 'this subject';
  var topic = topicTitle || 'this topic';
  var prompt =
    'Explain "' + topic + '" to me from scratch — but DO NOT just read my notes back. '
    + 'I am a Nigerian secondary school student preparing for WAEC/NECO. '
    + 'Use vivid, interesting Nigerian examples a student would actually recognise: '
    + 'okada, danfo, jollof rice, suya, NEPA, NYSC, GTBank, Dangote, market women in Onitsha, '
    + 'farmer in Kaduna, school assembly, harmattan, naira denomination, etc. '
    + 'Make it lively and memorable. Include at least 3 different Nigerian examples. '
    + 'If diagrams or visuals would help, describe them clearly with labels so they can be drawn. '
    + 'Break the explanation into short, easy paragraphs. End with one Nigerian-flavoured story '
    + 'or analogy that makes the concept stick. Subject: ' + subj + '. Student name: ' + firstName + '.';

  // Push into chat input and send
...
window.openExplainToMe = openExplainToMe;
...
async function startExamSession(){
  ...
  if (adaptive && adaptive.prompt){
    ...
    prompt = 'Generate exactly '+batchSize+' '+cfg.short+' '+subjName+' multiple-choice past-paper questions.\n\n'
      ...
      +'6. CRITICAL — SELF-CONTAINED QUESTIONS ONLY:\n'
      +'   • Every question must stand alone. Do NOT reference an external "passage", "diagram", "figure", "table", "graph", "map" unless you INCLUDE the full text/data INSIDE the question string itself.\n'
      +'   • For English comprehension: if you must use a passage, embed the FULL passage (200-300 words) at the start of the "q" field, then ask the question. Use \\n\\n line breaks.\n'
      +'   • For Maths/Physics/Chemistry: if a diagram is needed, describe it precisely in words within the question (or skip that question type).\n'
      +'   • NEVER write "According to the passage above" / "Look at the diagram" / "From the table" without the actual content inline.\n'
      +'   • If you cannot make a question self-contained, choose a DIFFERENT question type.\n\n'
      +'=== OUTPUT FORMAT ===\n'
      +'[{"yr":2022,"topic":"topic name","q":"Question text (include full passage/diagram description if any)","opts":{'+exampleOpts+'},"ans":"'+exampleAns+'","exp":"Why '+exampleAns+' is correct. Why other options are wrong.","diff":"medium"}]';
  }

  // Try Anthropic primary, then OpenAI fallback, then LOCAL skeleton.
  // Students should NEVER see a hard error — always get a usable paper.
  var raw = '';
  var questions = null;
  var systemPrompt = 'You are a senior '+cfg.longName+' ('+cfg.short+') examiner and past-paper specialist — you have marked '+cfg.short+' '+subjName+' papers for over a decade and know the exact house style, distractor patterns, and syllabus emphasis of THIS specific board. You do NOT confuse '+cfg.short+' with other Nigerian boards. You NEVER write questions in another board\'s style. Return ONLY raw JSON arrays when asked. Never include markdown or text outside the JSON.';

  async function tryProvider(endpoint, body){
    try {
      ...
      if (data && data.content && data.content.length){
        ...
        if (blk && blk.text) return blk.text;
      }
      // OpenAI shape
  ...
  try {
    // 1) Anthropic primary
    console.log('[ExamLoader] Trying Anthropic…');
    raw = await tryProvider('/api/anthropic', {
      model:'claude-sonnet-4-6',
      max_tokens:8000,
      system: systemPrompt,
      messages:[{role:'user',content: prompt}]
    });
    questions = raw ? fixExamJSON(raw) : null;

    // 2) OpenAI fallback if Anthropic returned nothing usable
    if (!questions || !questions.length){
      console.log('[ExamLoader] Anthropic empty — trying OpenAI fallback…');
      raw = await tryProvider('/api/openai', {
        model:'gpt-4o-mini',
        max_tokens:8000,
        messages:[
    ...
    // 3) Local skeleton fallback — last resort, never show an error to the student.
    if (!questions || !questions.length){
      console.warn('[ExamLoader] Both providers failed — switching to Lesson Teacher (local).');
      // Subtle notice — students see a friendly switch message, not an error
      showLocalFallbackToast();
      questions = buildLocalExamQuestions(cfg, subjName, batchSize);
    }
    ...
    // Even on exception, fall back to local rather than show an error
    showLocalFallbackToast();
    var fallbackQs = buildLocalExamQuestions(cfg, subjName, batchSize);
    examSession = {
      questions: fallbackQs.map(function(q){
        var norm = {};
        cfg.options.forEach(function(L){ norm[L] = q.opts[L] || ''; });
        return {
          year:'', topic:q.topic||'', tid:'', bloom:'',
          question:q.q, options:norm,
          answer:q.ans, explanation:q.exp||'', difficulty:'medium'
        };
      }),
      current:0,correct:0,answered:0,answers:[],
      timer:null,timeLeft:isTimed?numQ*60:0,
      subj:subjName,exam,mode:examMode,
      boardKey: currentExam, subjKey: currentExamSubj, topicAssignments: null
    };
    window._waecPaper=null;
    renderExamQuestion();
    if(isTimed) startExamTimer();
    var sc2=document.getElementById('examScoreChip');
    if(sc2){sc2.style.display='flex';sc2.textContent='0% · 0/'+fallbackQs.length;}
  }
}

// ════════════════════════════════════════════════════════════════
// SUBTLE LOCAL-FALLBACK NOTICE
// Shown when both AI providers fail. A small toast in the corner —
// students get a usable paper without seeing a scary error screen.
// ════════════════════════════════════════════════════════════════
function showLocalFallbackToast(){
  var existing = document.getElementById('ltLocalFallbackToast');
  if (existing) existing.remove();
  var t = document.createElement('div');
  t.id = 'ltLocalFallbackToast';
  t.style.cssText = 'position:fixed;bottom:max(20px,env(safe-area-inset-bottom));left:50%;'
    + 'transform:translateX(-50%);background:rgba(30,58,138,.95);color:#fff;'
    + 'padding:10px 18px;border-radius:100px;font-family:inherit;font-size:.82rem;'
    + 'font-weight:700;border:1px solid rgba(251,191,36,.45);'
    + 'box-shadow:0 8px 28px rgba(0,0,0,.45);z-index:99990;'
    + 'display:flex;align-items:center;gap:8px;opacity:0;transition:opacity .3s';
  t.innerHTML = '<span>💡</span><span>Switching to Lesson Teacher (local) — paper still ready for you.</span>';
  document.body.appendChild(t);
...
window.showLocalFallbackToast = showLocalFallbackToast;

// ════════════════════════════════════════════════════════════════
// LOCAL EXAM-QUESTION BUILDER (skeleton paper)
// Used only when both AI providers fail. Builds simple multi-choice
// questions from generic syllabus prompts so the student can still
// practise instead of seeing an error.
// ════════════════════════════════════════════════════════════════
function buildLocalExamQuestions(cfg, subjName, n){
  var topics = (function(){
    if (typeof window.getAllTopics === 'function'){
      var t = window.getAllTopics(currentExam, currentExamSubj);
      if (t && t.length) return t.map(function(x){ return x.name || x.title || x; });
    }
    return [
      'Foundational concepts', 'Definitions and terminology',
      'Practical applications', 'Nigerian-context examples',
      'Past-paper themes', 'Key formulas / rules', 'Common errors',
      'Comparison of ideas', 'Cause and effect', 'Modern relevance'
    ];
  })();
  var out = [];
  for (var i = 0; i < n; i++){
    ...
    var opts = {};
    cfg.options.forEach(function(L, idx){
      opts[L] = ['Correct understanding of ' + topic,
                 'Partial understanding only',
                 'Common misconception about ' + topic,
                 'Unrelated concept',
                 'Outdated view of ' + topic][idx] || ('Option ' + L);
    });
    out.push({
      yr: new Date().getFullYear(),
      topic: topic,
      q: 'Practice question on "' + topic + '" — pick the best answer that reflects how this concept applies in the ' + subjName + ' syllabus.',
      opts: opts,
      ans: cfg.options[0],
      exp: 'The correct answer captures the core principle. Review your notes on ' + topic + ' for full mastery.',
      diff: 'medium'
    });
  }
  return out;
}
window.buildLocalExamQuestions = buildLocalExamQuestions;
...
function renderExamResults(){
  ...
  // Mark all questions in this session as "seen" so the student won't see
  // them again next time they take this board+subject exam.
  try {
    if (typeof markQuestionsAsSeen === 'function' && examSession.questions){
      var boardKey = (examSession.exam || currentExam || 'waec').toLowerCase();
      var subjKey  = (currentExamSubj || '').replace(/-[a-z0-9]+$/, '');
      markQuestionsAsSeen(boardKey, subjKey, examSession.questions);
    }
  } catch(e){ console.warn('seen-tracking failed', e); }
...
function toggleTool(id){
  ...
  if(!isOpen){
    ...
    // Inject a backdrop so users can tap outside to close + return to question
    if(!document.getElementById('toolBackdrop')){
      var bd = document.createElement('div');
      bd.id = 'toolBackdrop';
      bd.style.cssText = 'position:fixed;inset:0;z-index:499;background:rgba(0,0,0,.5);backdrop-filter:blur(2px);cursor:pointer;';
      bd.title = 'Tap to close and return to question';
      bd.onclick = function(){ closeTool(id); };
      document.body.appendChild(bd);
    }
  } else {
    // Closing — remove backdrop too
    var bd2 = document.getElementById('toolBackdrop');
    if (bd2) bd2.remove();
  }
...
function closeTool(id){
  ...
  // Remove tool backdrop if no other tools are still open
  var anyOpen = document.querySelector('.tool-panel.open');
  if (!anyOpen){
    var bd = document.getElementById('toolBackdrop');
    if (bd) bd.remove();
  }
}
...
function buildSubjectToolkit(subjKey){
  ...
  // Objective is multi-choice — students PICK an answer; they don't write.
  // So we only show tools that genuinely help SOLVE a multi-choice question
  // (calculator, formula sheet, periodic table, etc.). Writing tools
  // (outliner, dictation, wordcount, quotebox) are excluded — they belong in theory mode.
  var OBJ_TOOLKITS = {
    ...
    bio:  ['punnett','diagram'],
    eng:  [], // no tools in English objective — pure reading + comprehension
    lit:  [], // no tools in Literature objective
    eco:  ['calculator','graph','formulasheet'],
    govt: [], // no tools in Government objective
    geo:  ['calculator','nigeriamap'],
    crs:  [], // no tools in CRS objective
    irs:  [],
    agr:  ['calculator'],
  ...
  // Always-present submit button (even when no subject tools)
  html += '<button class="etk-btn" style="' + (renderedAny ? 'margin-left:auto;' : '') + 'background:rgba(224,58,47,.12);border-color:rgba(224,58,47,.3);color:#fca5a5" onclick="if(confirm(\'End exam and submit?\'))renderExamResults()">↳ Submit Paper</button>';
  html += '</div>';
  return html;
}
