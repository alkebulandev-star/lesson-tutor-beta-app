...
async function fetchAnthropic(body, opts){
  ...
  // If we didn't ask for streaming, OR the server returned JSON instead
  // of SSE (e.g. local dev server), parse as plain JSON.
  var _ct = (resp.headers.get('content-type') || '').toLowerCase();
  if (!streaming || _ct.indexOf('text/event-stream') === -1){
