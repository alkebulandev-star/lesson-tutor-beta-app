...
(function(){
  ...
  function run(tourId, opts){
    ...
    // When user ticks "Don't show again", IMMEDIATELY close + persist
    // (don't wait for them to also tap Skip/Next).
    ov.addEventListener('change', function(e){
      if (e.target && e.target.id === 'lt-spot-dont' && e.target.checked){
        close(tourId, true);
      }
    });
